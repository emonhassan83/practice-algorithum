...
void dsu_union_by_size(int node1, int node2)
{
    ...
    if(leaderA == leaderB) return; //* if find same leader then return
    ...
    if (group_size[leaderA] > group_size[leaderB])
    {
        ...
        mx= max(mx, group_size[leaderA]); //* update mx size
    }
    else
    {
        par[leaderA] = leaderB;
        ...
        mx= max(mx, group_size[leaderB]); //* update mx size
    }
    cmp--; //* update component size
}
